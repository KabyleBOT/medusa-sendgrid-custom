...
class SendGridService extends NotificationService {
   ...
   *      customer_created_template: 2222,
   *      email_verification_template: 4444,
   *    }
   */
  constructor(
    {
  ...
  async fetchData(event, eventData, attachmentGenerator) {
    switch (event) {
      ...
      case 'customer.created':
        return this.customerCreated(eventData, attachmentGenerator);
      case 'user.created':
        return this.userCreated(eventData, attachmentGenerator);
  ...
  getLocalizedTemplateId(event, locale) {
    if (this.options_.localization && this.options_.localization[locale]) {
      ...
      switch (event) {
        ...
        case 'user.created':
          return map.user_created_template;
        case 'customer.password_reset':
  ...
  getTemplateId(event) {
    switch (event) {
      ...
      case 'user.created':
        return this.options_.user_created_template;
      case 'customer.password_reset':
  ...
  async orderPlacedData({ id }) {
    ...
    const taxRate = order.region.tax_rate / 100;
    const nbr_items = order.items.reduce((acc, i) => acc + i.quantity, 0);
    let nbr_item = false;
    if (nbr_items === 1) {
      nbr_item = true;
    }
    ...
    const items = await Promise.all(
      order.items.map(async (i) => {
        ...
        if (!variant) {
          this.logger_.error(`Variant with ID ${i.variant_id} not found`);
          return i;
        }
        i.options = variant.options.map((o) => {
          const option = product.options.find((opt) => opt.id === o.option_id);

          if (!option) {
            this.logger_.error(`Option with ID ${o.option_id} not found`);
            return {
